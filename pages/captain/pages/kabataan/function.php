...
if(isset($_POST['btn_add'])){
    ...
    if(isset($_SESSION['role'])){
        $action = 'Added SK named '.$txt_fname.', '.$txt_mname.' '.$txt_lname;
        $iquery = mysqli_query($con,"INSERT INTO tbllogs (user,logdate,action) values ('".$_SESSION['role']."', NOW(), '".$action."')");
    }

    $su = mysqli_query($con,"SELECT * from tblkabataan where fname = '".$txt_fname."' ");
    ...
    if($ct == 0){
        ...
        if($name != ""){
            if(($imagetype=="image/jpeg" || $imagetype=="image/png" || $imagetype=="image/bmp") && $size<=2048000){
                ...
                {
                    $txt_image = $image;
                    $query = mysqli_query($con,"INSERT INTO tblkabataan (
                                        fname,
                                        mname,
                                        lname,
                                        position,
...
if(isset($_POST['btn_save'])){
    ...
    $su = mysqli_query($con,"SELECT * from tblkabataan where id = '".$txt_id."' and id not in (".$txt_id.") ");
    $ct = mysqli_num_rows($su);
    ...
    if($ct == 0){
        ...
        if($name != ""){
            if(($imagetype=="image/jpeg" || $imagetype=="image/png" || $imagetype=="image/bmp") && $size<=2048000){
                ...
                $update_query = mysqli_query($con,"UPDATE tblkabataan set 
            ...
            $chk_image = mysqli_query($con,"SELECT * from tblkabataan where id = '".$_POST['hidden_id']."' ");
        ...
        if(isset($_POST['chk_delete'])){
            foreach($_POST['chk_delete'] as $value){
                $delete_query = mysqli_query($con, "DELETE from tblkabataan where id = '$value' ") or die('Error: ' . mysqli_error($con));
