...
if(isset($_POST['btn_add'])){
    ...
    $su = mysqli_query($con,"SELECT * from tblkawani where fname = '".$txt_fname."' ");
    ...
    if($ct == 0){
        ...
        if($name != ""){
            if(($imagetype=="image/jpeg" || $imagetype=="image/png" || $imagetype=="image/bmp") && $size<=2048000){
                ...
                {
                    $txt_image = $image;
                    $query = mysqli_query($con,"INSERT INTO tblkawani (
                                        fname,
                                        mname,
                                        lname,
                                        position,
...
if(isset($_POST['btn_save'])){
    ...
    $su = mysqli_query($con,"SELECT * from tblkawani where id = '".$txt_edit_fname."' and id not in (".$txt_id.") ");
    $ct = mysqli_num_rows($su);
    ...
    if($ct == 0){
        ...
        if($name != ""){
            if(($imagetype=="image/jpeg" || $imagetype=="image/png" || $imagetype=="image/bmp") && $size<=2048000){
                ...
                $update_query = mysqli_query($con,"UPDATE tblkawani set 
            ...
            $chk_image = mysqli_query($con,"SELECT * from tblkawani where id = '".$_POST['hidden_id']."' ");
        ...
        if(isset($_POST['chk_delete'])){
            foreach($_POST['chk_delete'] as $value){
                $delete_query = mysqli_query($con, "DELETE from tblkawani where id = '$value' ") or die('Error: ' . mysqli_error($con));
